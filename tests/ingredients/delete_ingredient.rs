...
use just_recipe::{application::{app::App, state::AppState}, utilities::{random_generation::ingredients::choose_random_ingredient, request_creators::create_delete_request_to}};
use serde_json::json;
use sqlx::PgPool;
...
async fn deleting_existing_ingredient_gets_removed_returns_204_no_content(
    ...
    let ingredient_id = choose_random_ingredient
        .await
        .ingredient_id;
    let request = create_delete_request_to("ingredients", json!({"ingredient_id": ingredient_id}));
    let response = app.router.oneshot(request).await.unwrap();
