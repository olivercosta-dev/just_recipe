...
async fn adding_new_recipe_persists_and_returns_200_ok(pool: PgPool) -> sqlx::Result<()> {
    ...
    assert_eq!(ordered_recipe_step_records.len(), steps.len(),"Step numbers should match");
    ...
    for (index, step) in steps.iter().enumerate() {
        let step_number = i32::try_from(step["step_number"].as_i64().expect("Should have been an integer")).expect("Should have been an i32");
        let instruction = String::from(step["instruction"].as_str().expect("Should have been a string"));
        let recipe_step_record = &ordered_recipe_step_records[index];
        let (record_step_number, record_instruction) = (recipe_step_record.step_number.unwrap(), (recipe_step_record.instruction.clone().unwrap()));
        assert_eq!(
...
async fn adding_recipe_with_non_existent_ingredient_id_returns_422_unproccessable_entity(pool: PgPool) -> sqlx::Result<()>{
    let app_state = AppState { pool };
    let app = new_app(app_state.clone()).await;
    let recipe_name = Faker.fake::<String>();
    let description = Faker.fake::<String>();
